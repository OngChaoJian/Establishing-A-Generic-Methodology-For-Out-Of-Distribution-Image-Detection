...
# Test Methodology
def RunTestMethod(modelList, masterTreeList, runList, args, useFixedSoftmaxThreshold, fixedSoftmaxValue):
    # Define parameters
    ...
    for run in runList:
        ...
        testLoader = None
        if run == "Cifar10_validation":
            with open(os.path.join(args.cifar_dir, "validation_indices"), 'rb') as f:
                indices = pickle.load(f)
            testSampler = SubsetRandomSampler(indices)
            testLoader = DataLoader(dset.CIFAR10(root=args.cifar_dir, train=False, download=True, transform=testTransformer), 
                batch_size=1, sampler=testSampler, shuffle=False, num_workers=4)
        elif run == "Cifar10_test":
            with open(os.path.join(args.cifar_dir, "test_indices"), 'rb') as f:
                indices = pickle.load(f)
            testSampler = SubsetRandomSampler(indices)
            testLoader = DataLoader(dset.CIFAR10(root=args.cifar_dir, train=False, download=True, transform=testTransformer), 
                batch_size=1, sampler=testSampler, shuffle=False, num_workers=4)
        else:
            testDataset = torchvision.datasets.ImageFolder(os.path.join(args.ood_dir, dataName), transform=testTransformer)
...
def main(args):
    ...
    RunTestMethod(modelList, masterTreeList, runList, args, args.use_fixed_softmax_threshold, args.fixed_softmax_threshold_value)
